...
import com.fiveguysburger.emodiary.core.dto.DiaryMessages
...
class ChatRoomServiceImpl(
    ...
    override fun getAllMessages(
        ...
        val dirtyJsonString: String =
            directToolCall.findDocuments(
                collection = "messages",
                filter = filter,
                limit = 100,
        ...
        val outerList: List<Map<String, String>> =
        ...
        val diaryMessagesDtoList: List<DiaryMessages> =
            objectMapper.readValue(
                contentString,
    ...
    override fun requestDiaryAnalysis(
        chatroomId: String,
        userId: String,
        ...
        val chatResponse =
            chatClient.prompt()
                .system(
                    "너는 EmoDiary의 다정한 친구 '감정이'야. " +
                            "대화 내용을 분석하고 사용자의 감정을 짚어주는 공감 능력이 뛰어난 일기 분석 전문가야. " +
                            "시간의 흐름에 따라 감정이 어떻게 변화했는지 구체적인 예시를 들어 설명해줘. " +
                            "그리고 사용자의 말에 깊이 공감하는 문장을 포함해줘. 마지막으로, 따뜻한 응원과 덕담을 덧붙여줘.",
                )
